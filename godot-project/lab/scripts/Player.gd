...
onready var Pause = load("res://lab/scenes/Pause.tscn")
export var Speed = 10
...
func _process(_delta):
	...
	if $Head/Camera/RayCast.is_colliding():
		...
		if Collider.is_in_group("Element"):
			...
			Pickable = true
			$Crosshair.texture = load("res://lab/assets/Hand.png")
	else:
		$Label.text = ''
		Pickable = false
		$Crosshair.texture = load("res://lab/assets/Crosshair.png")
		...
		if Pickable == true:
			...
			if Collider.is_in_group('Element'):
				...
				if data['Slot'+str(Scroll)] == '':
					...
					get_node("Hotbar/GridContainer/Panel"+str(Scroll)+"/Sprite").texture = load("res://lab/assets/"+data['Slot'+str(Scroll)]+".png")
					get_node("Head/Camera/Sprite3D").texture = load("res://lab/assets/"+data['Slot'+str(Scroll)]+".png")
					$Crosshair.texture = load("res://lab/assets/Crosshair.png")
					file.close()

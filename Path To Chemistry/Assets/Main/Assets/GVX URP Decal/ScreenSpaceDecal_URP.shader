...
#define sampler_BumpMap sampler_NormalMap
				...
				half4 LitPassFragment(Varyings input) : SV_Target
				{
					...
					half3 vd = GetCameraPositionWS() - positionWS;
					half3 cd = mul((float3x3)unity_CameraToWorld, float3(0, 0, 1));
					float sceneZ = LinearEyeDepth(SampleSceneDepth(input.uv), _ZBufferParams);
					float3 wpos = GetCameraPositionWS() + sceneZ * (vd / dot(vd, cd));

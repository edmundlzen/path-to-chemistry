...
public class DragonSoulEaterEnemy: MonoBehaviour, IEntity
{
    ...
    { 
        ...
        audioSource = GetComponent<AudioSource>();
        pFlame = transform.Find("Flame origin").Find("PFlame").gameObject;
    ...
    void Update()
    {
        ...
        switch (currentState)
        {
            ...
            case EntityStates.Attack:
                ...
                float animatorTime = animationState.normalizedTime - (float)Math.Truncate(animationState.normalizedTime);
                if (animatorTime > 0.5f && animatorTime < 0.65f)
                {
                    // pFlame.transform.LookAt(player);
                    // pSmoke.transform.LookAt(player);
                    // pSpark.transform.LookAt(player);
                    EnableFlame();
                }
                else
                {

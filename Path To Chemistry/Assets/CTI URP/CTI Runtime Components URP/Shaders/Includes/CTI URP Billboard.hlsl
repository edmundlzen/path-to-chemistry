...
void CTIBillboardVert(inout CTIVertexBBInput v, float3 lightDir)
{
    ...
    float4 vOscillations = CTISmoothTriangleWave(float4(TreeWorldPos.x + sinuswave, TreeWorldPos.z + sinuswave * 0.8,
                                                        0.0, 0.0));
    float fOsc = vOscillations.x + (vOscillations.y * vOscillations.y);
    fOsc = 0.75 + (fOsc + 3.33) * 0.33;

    //	saturate added to stop warning on dx11...
    v.positionOS.xyz += _CTI_SRP_Wind.w * _CTI_SRP_Wind.xyz * _WindStrength * fOsc * pow(saturate(percent.y), 1.5);
    // pow(y,1.5) matches the wind baked to the mesh trees

...
namespace Utils
{
    ...
    public class serializableUnityARMatrix4x4
    {
        ...
        public serializableUnityARMatrix4x4(SerializableVector4 v0, SerializableVector4 v1, SerializableVector4 v2,
            SerializableVector4 v3)
        {
            column0 = v0;
            column1 = v1;
            column2 = v2;
        ...
        {
            return new serializableUnityARMatrix4x4(rValue.GetColumn(0), rValue.GetColumn(1), rValue.GetColumn(2),
                rValue.GetColumn(3));
        }
    ...
    public class serializableSHC
    {
        ...
        public static implicit operator serializableSHC(float[] floatsSHC)
        {
            if (floatsSHC != null)
            {
                var createBuf = new byte[floatsSHC.Length * sizeof(float)];
                for (var i = 0; i < floatsSHC.Length; i++)
                    Buffer.BlockCopy(BitConverter.GetBytes(floatsSHC[i]), 0, createBuf, i * sizeof(float),
                        sizeof(float));
        ...
        public static implicit operator float[](serializableSHC spc)
        {
            if (spc.shcData != null)
            {
                var numFloats = spc.shcData.Length / sizeof(float);
                var shcFloats = new float[numFloats];
    ...
    public class serializableUnityARLightData
    {
        ...
        private serializableUnityARLightData(UnityARLightData lightData)
        {
            ...
            if (whichLight == LightDataType.DirectionalLightEstimate)
            {
                ...
                var lightDir = lightData.arDirectonalLightEstimate.primaryLightDirection;
                var lightIntensity = lightData.arDirectonalLightEstimate.primaryLightIntensity;
                primaryLightDirAndIntensity =
                    new SerializableVector4(lightDir.x, lightDir.y, lightDir.z, lightIntensity);
            }
            else
            {
        ...
        public static implicit operator UnityARLightData(serializableUnityARLightData rValue)
        {
            ...
            var ule = new UnityARLightEstimate(rValue.ambientIntensity, rValue.ambientColorTemperature);

            if (rValue.whichLight == LightDataType.DirectionalLightEstimate)
            {
                var lightDir = new Vector3(rValue.primaryLightDirAndIntensity.x, rValue.primaryLightDirAndIntensity.y,
                    rValue.primaryLightDirAndIntensity.z);
                udle = new UnityARDirectionalLightEstimate(rValue.lightSHC, lightDir,
                    rValue.primaryLightDirAndIntensity.w);
            }
    ...
    public class serializableUnityARCamera
    {
        ...
        public serializableUnityARCamera(serializableUnityARMatrix4x4 wt, serializableUnityARMatrix4x4 pm,
            ARTrackingState ats, ARTrackingStateReason atsr, UnityVideoParams uvp, UnityARLightData lightDat,
            serializableUnityARMatrix4x4 dt, serializablePointCloud spc)
        {
            worldTransform = wt;
            projectionMatrix = pm;
            trackingState = ats;
        ...
        {
            return new serializableUnityARCamera(rValue.worldTransform, rValue.projectionMatrix, rValue.trackingState,
                rValue.trackingReason, rValue.videoParams, rValue.lightData, rValue.displayTransform,
                rValue.pointCloudData);
        }

        public static implicit operator UnityARCamera(serializableUnityARCamera rValue)
        {
            return new UnityARCamera(rValue.worldTransform, rValue.projectionMatrix, rValue.trackingState,
                rValue.trackingReason, rValue.videoParams, rValue.lightData, rValue.displayTransform,
                rValue.pointCloud);
        }
    }
    ...
    public class serializableUnityARPlaneAnchor
    {
        ...
        public serializableUnityARPlaneAnchor(serializableUnityARMatrix4x4 wt, SerializableVector4 ctr,
            SerializableVector4 ext, ARPlaneAnchorAlignment apaa,
            byte[] idstr)
        {
            worldTransform = wt;
            center = ctr;
        ...
        public static implicit operator serializableUnityARPlaneAnchor(ARPlaneAnchor rValue)
        {
            ...
            var ctr = new SerializableVector4(rValue.center.x, rValue.center.y, rValue.center.z, 1.0f);
            var ext = new SerializableVector4(rValue.extent.x, rValue.extent.y, rValue.extent.z, 1.0f);
            var idstr = Encoding.UTF8.GetBytes(rValue.identifier);
    ...
    public class serializablePointCloud
    {
        ...
        public static implicit operator serializablePointCloud(Vector3[] vecPointCloud)
        {
            ...
            {
                var createBuf = new byte[vecPointCloud.Length * sizeof(float) * 3];
                for (var i = 0; i < vecPointCloud.Length; i++)
                {
                    var bufferStart = i * 3;
                    Buffer.BlockCopy(BitConverter.GetBytes(vecPointCloud[i].x), 0, createBuf,
                        bufferStart * sizeof(float), sizeof(float));
                    Buffer.BlockCopy(BitConverter.GetBytes(vecPointCloud[i].y), 0, createBuf,
                        (bufferStart + 1) * sizeof(float), sizeof(float));
                    Buffer.BlockCopy(BitConverter.GetBytes(vecPointCloud[i].z), 0, createBuf,
                        (bufferStart + 2) * sizeof(float), sizeof(float));
                }
        ...
        public static implicit operator Vector3[](serializablePointCloud spc)
        {
            ...
            {
                var numVectors = spc.pointCloudData.Length / (3 * sizeof(float));
                var pointCloudVec = new Vector3[numVectors];
    ...
    public class serializableARSessionConfiguration
    {
        ...
        public serializableARSessionConfiguration(UnityARAlignment align, UnityARPlaneDetection planeDet,
            bool getPtCloud, bool enableLightEst)
        {
            alignment = align;
        ...
        {
            return new serializableARSessionConfiguration(awtsc.alignment, awtsc.planeDetection,
                awtsc.getPointCloudData, awtsc.enableLightEstimation);
        }

        public static implicit operator ARKitWorldTrackingSessionConfiguration(serializableARSessionConfiguration sasc)
        {
            return new ARKitWorldTrackingSessionConfiguration(sasc.alignment, sasc.planeDetection,
                sasc.getPointCloudData, sasc.enableLightEstimation);
        }
    }

...
public class FocusSquare : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        var center = new Vector3(Screen.width / 2, Screen.height / 2, findingSquareDist);
        ...
        if (Physics.Raycast(ray, out hit, maxRayDistance, collisionLayerMask))
        {
            ...
            Debug.Log(string.Format("x:{0:0.######} y:{1:0.######} z:{2:0.######}", foundSquare.transform.position.x,
                foundSquare.transform.position.y, foundSquare.transform.position.z));
        ...
        if (trackingInitialized)
        {
            ...
            if (Vector3.Dot(Camera.main.transform.forward, Vector3.down) > 0)
            {
                ...
                var vecToCamera = findingSquare.transform.position - Camera.main.transform.position;
        ...
        if (hitResults.Count > 0)
            foreach (var hitResult in hitResults)
            {
                ...
                Debug.Log(string.Format("x:{0:0.######} y:{1:0.######} z:{2:0.######}",
                    foundSquare.transform.position.x, foundSquare.transform.position.y,
                    foundSquare.transform.position.z));
                return true;
            }

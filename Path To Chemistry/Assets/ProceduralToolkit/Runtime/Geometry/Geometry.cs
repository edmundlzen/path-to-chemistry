...
namespace ProceduralToolkit
{
    ...
    public static class Geometry
    {
        ...
        {
            var segmentAngle = 360f / vertices;
        ...
        {
            var toPrevious = (previous - current).normalized;
        ...
        public static List<Vector2> OffsetPolygon(IList<Vector2> polygon, float distance)
        {
            ...
            for (var i = 0; i < polygon.Count; i++)
            {
                ...
                var bisector = GetAngleBisector(previous, current, next, out var angle);
                var angleOffset = GetAngleOffset(distance, angle);
        ...
        public static void OffsetPolygon(ref List<Vector2> polygon, float distance)
        {
            ...
            for (var i = 0; i < polygon.Count; i++) polygon[i] += offsets[i];
        }
        ...
        public static void OffsetPolygon(ref Vector2[] polygon, float distance)
        {
            ...
            for (var i = 0; i < polygon.Length; i++) polygon[i] += offsets[i];
        }
        ...
        public static List<Vector2> PointsOnSegment2(Vector2 segmentA, Vector2 segmentB, int count)
        {
            ...
            for (var i = 0; i < count; i++) points.Add(PointOnSegment2(segmentA, segmentB, i / (float) (count - 1)));
            return points;
        }
        ...
        public static List<Vector2> PointsInCircle2(Vector2 center, float radius, int count)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                // The 0.5 offset improves the position of the first point
                var r = Mathf.Sqrt((i + 0.5f) / count);
                points.Add(center + new Vector2(radius * Mathf.Sin(currentAngle) * r,
                    radius * Mathf.Cos(currentAngle) * r));
                currentAngle += PTUtils.GoldenAngle;
            }
        ...
        public static List<Vector3> PointsOnSegment3(Vector3 segmentA, Vector3 segmentB, int count)
        {
            ...
            for (var i = 0; i < count; i++) points.Add(PointOnSegment3(segmentA, segmentB, i / (float) (count - 1)));
            return points;
        }
        ...
        public static Vector3 PointOnTeardrop(float radius, float height, float horizontalAngle, float verticalAngle)
        {
            var horizontalRadians = horizontalAngle * Mathf.Deg2Rad;
            var verticalRadians = verticalAngle * Mathf.Deg2Rad;
            var sinVertical = Mathf.Sin(verticalRadians);
            var teardrop = (1 - sinVertical) * Mathf.Cos(verticalRadians) / 2;

            return new Vector3(
                radius * Mathf.Sin(horizontalRadians) * teardrop,
                height * sinVertical,
                radius * Mathf.Cos(horizontalRadians) * teardrop);
        }
        ...
        public static List<Vector3> PointsOnSphere(float radius, int count)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                points.Add(new Vector3(
                    radius * Mathf.Sin(currentAngle) * r,
                    radius * y,
                    radius * Mathf.Cos(currentAngle) * r));
                y += deltaY;
                currentAngle += PTUtils.GoldenAngle;

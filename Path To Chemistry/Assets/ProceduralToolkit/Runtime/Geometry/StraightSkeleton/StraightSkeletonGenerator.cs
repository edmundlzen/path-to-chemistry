    ...
    public class StraightSkeletonGenerator
    {
        ...
        public StraightSkeleton Generate(IList<Vector2> polygon)
        {
            ...
            while (activePlans.Count > 0)
            {
                ...
                var success = OffsetAndDetectEvents();
                if (!success) return skeleton;
                count++;
            }
        ...
        private bool OffsetAndDetectEvents()
        {
            ...
            {
                var offset = FindSmallestOffset(plan);
                ...
                foreach (var intersectionEvent in intersectionEvents) ProcessIntersectionEvent(plan, intersectionEvent);
        ...
        {
            var smallestOffset = float.PositiveInfinity;
            foreach (var vertex in plan)
            {
                if (BisectorBisector(vertex, vertex.next, out var offset))
                ...
                if (vertex.reflect)
                    foreach (var segmentA in plan)
                    {
                        ...
                        if (segmentA == vertex || segmentB == vertex) continue;
        ...
        private List<IntersectionEvent> FindIntersectionEvents(Plan plan)
        {
            ...
            do
            {
                ...
                if (intersectionEvent != null)
                {
                    ...
                    if (intersectionEvent.chains[0].Count == plan.Count) return intersectionEvents;
                }
        ...
        {
            foreach (var chain in intersectionEvent.chains) SimplifyChain(plan, chain);
        ...
        private static void CalculateBisector(Plan.Vertex vertex)
        {
            ...
            var direction = Geometry.GetAngleBisector(previous.position, vertex.position, next.position, out var angle);
            vertex.angle = angle;
        ...
        private static bool BisectorBisector(Plan.Vertex vertexA, Plan.Vertex vertexB, out float offset)
        {
                ...
                out var intersection))
            {
                ...
                {
                    var offsetA = GetBisectorBisectorOffset(vertexA, intersection.pointA);
                    var offsetB = GetBisectorBisectorOffset(vertexB, intersection.pointA);
                ...
                {
                    var toIntersection = Vector2.Distance(vertexA.position, vertexB.position);
                    var offsetA = GetBisectorBisectorOffset(vertexA, toIntersection) / 2;
                    var offsetB = GetBisectorBisectorOffset(vertexB, toIntersection) / 2;
                    offset = Mathf.Min(offsetA, offsetB);
                    return true;
                }
        ...
        {
            var toIntersection = Vector2.Distance(vertex.position, intersection);
        ...
        private static bool BisectorSegment(Plan.Vertex vertex, Vector2 segmentA, Vector2 segmentB, out float offset)
        {
            if (Intersect.RaySegment(vertex.position, vertex.bisector, segmentA, segmentB, out var intersection))
            {
                if (intersection.type == IntersectionType.Point)
                {
                    var segmentDirection = (segmentB - segmentA).normalized;
                    var toIntersection = Vector2.Distance(vertex.position, intersection.pointA);
                    var intersectionAngle = Vector2.Angle(vertex.bisector, segmentDirection);
                    var intersectionSin = Mathf.Sin(intersectionAngle * Mathf.Deg2Rad);
                    var bisectorSin = Geometry.GetAngleBisectorSin(vertex.angle);

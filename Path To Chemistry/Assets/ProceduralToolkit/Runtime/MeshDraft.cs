...
namespace ProceduralToolkit
{
    ...
    public partial class MeshDraft
    {
        ...
        public MeshDraft Add(MeshDraft draft)
        {
            ...
            for (var i = 0; i < draft.triangles.Count; i++) triangles.Add(draft.triangles[i] + vertices.Count);
            ...
            bool reverseTriangles = false)
        {
            ...
            else
            {
                if (reverseTriangles)
                {
                    for (var i = ring.Count - 1; i > 0; i--) AddTriangle(ring[i - 1], apex, ring[i], true);
                    AddTriangle(ring[ring.Count - 1], apex, ring[0], true);
                }
                else
                {
                    for (var i = 0; i < ring.Count - 1; i++) AddTriangle(ring[i + 1], apex, ring[i], true);
        ...
        {
            if (lowerRing.Count != upperRing.Count) throw new ArgumentException("Array sizes must be equal");
            if (lowerRing.Count < 3) throw new ArgumentException("Array sizes must be greater than 2");

            var uv00 = new Vector2(0, 0);
            var uvBottomCenter = new Vector2(0.5f, 0);
            var uv10 = new Vector2(1, 0);
            var uv01 = new Vector2(0, 1);
            var uvTopCenter = new Vector2(0.5f, 1);
            var uv11 = new Vector2(1, 1);

            Vector3 lower0, upper0, lower1, upper1;
            for (var i = 0; i < lowerRing.Count - 1; i++)
            {
                lower0 = lowerRing[i];
        ...
        {
            for (var i = 0; i < vertices.Count; i++) vertices[i] += vector;
            return this;
        }
        ...
        {
            colors.Clear();
            for (var i = 0; i < vertices.Count; i++) colors.Add(color);
            return this;
        }
        ...
        public MeshDraft FlipUVHorizontally(int channel = 0)
        {
            ...
            for (var i = 0; i < list.Count; i++) list[i] = new Vector2(1 - list[i].x, list[i].y);
            return this;
        }
        ...
        public MeshDraft FlipUVVertically(int channel = 0)
        {
            ...
            for (var i = 0; i < list.Count; i++) list[i] = new Vector2(list[i].x, 1 - list[i].y);
            return this;
        }
        ...
        private void FillMesh(ref Mesh mesh, bool calculateBounds, bool autoIndexFormat)
        {
            ...
            {
                if (autoIndexFormat) mesh.indexFormat = IndexFormat.UInt16;
            }
        ...
        {
            var vertex0 = origin;
            var vertex1 = origin + height;
            var vertex2 = origin + height + width;
            ...
            bool calculateNormal)
        {
            if (calculateNormal)
            {
                var normal = Vector3.Cross(vertex1 - vertex0, vertex3 - vertex0).normalized;
        ...
        {
            var normal = Vector3.Cross(fan[1] - fan[0], fan[fan.Count - 1] - fan[0]).normalized;
        ...
        public MeshDraft AddTriangleFan(IList<Vector3> fan, Vector3 normal, bool reverseTriangles = false)
        {
            ...
            for (var i = 0; i < fan.Count; i++) normals.Add(normal);
            return this;
        }
        ...
        {
            var normal = Vector3.Cross(strip[1] - strip[0], strip[2] - strip[0]).normalized;
        ...
        public MeshDraft AddTriangleStrip(IList<Vector3> strip, Vector3 normal)
        {
            ...
            for (var i = 0; i < strip.Count; i++) normals.Add(normal);
            return this;
        }

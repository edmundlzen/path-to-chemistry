    ...
    public static class ArrayE
    {
            ...
            IEqualityComparer<T> comparer = null)
        {
            ...
            void process(int x, int y)
            {
                if (!processed[x, y])
                {
                    if (comparer.Equals(array[x, y], value)) queue.Enqueue(new Vector2Int(x, y));
                    processed[x, y] = true;
                }
            }
            ...
            while (queue.Count > 0)
            {
                ...
                if (cell.x > 0) process(cell.x - 1, cell.y);
                if (cell.x + 1 < lengthX) process(cell.x + 1, cell.y);
                if (cell.y > 0) process(cell.x, cell.y - 1);
                if (cell.y + 1 < lengthY) process(cell.x, cell.y + 1);
        ...
        public static void Visit4<T>(this T[,] array, int x, int y, Action<int, int> visit)
        {
            ...
            if (x > 0) visit(x - 1, y);
            if (x + 1 < array.GetLength(0)) visit(x + 1, y);
            if (y > 0) visit(x, y - 1);
            if (y + 1 < array.GetLength(1)) visit(x, y + 1);
        }
        ...
        public static void Visit8<T>(this T[,] array, int x, int y, Action<int, int> visit)
        {
            ...
            var xLessThanWidth = x + 1 < array.GetLength(0);

            var yGreaterThanZero = y > 0;
            var yLessThanHeight = y + 1 < array.GetLength(1);

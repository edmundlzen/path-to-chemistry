...
namespace ProceduralToolkit
{
    ...
    public partial class MeshDraft
    {
        ...
        public static MeshDraft Plane(float xSize = 1, float zSize = 1, int xSegments = 1, int zSegments = 1,
            bool generateUV = true)
        {
            var xStep = xSize / xSegments;
            var zStep = zSize / zSegments;
            ...
            for (var x = 0; x <= xSegments; x++)
            {
                ...
                if (generateUV) draft.uv.Add(new Vector2((float) x / xSegments, (float) z / zSegments));
            }
        ...
        public static MeshDraft Pyramid(float radius, int segments, float height, bool generateUV = true)
        {
            ...
            if (generateUV)
            {
                ...
                for (var i = 0; i < segments; i++)
                {
                    var uv = Geometry.PointOnCircle2(0.5f, currentAngle) + new Vector2(0.5f, 0.5f);
                    uv.x = 1 - uv.x;
                    fanUV[i] = uv;
                    currentAngle += segmentAngle;
                }
        ...
        public static MeshDraft Prism(float radius, int segments, float height, bool generateUV = true)
        {
            var segmentAngle = 360f / segments;
            float currentAngle = 0;
            var halfHeightUp = Vector3.up * height / 2;
        ...
        public static MeshDraft Cylinder(float radius, int segments, float height, bool generateUV = true)
        {
            ...
            for (var i = 0; i < segments; i++)
            {
                AddCylinderPoints(radius, currentAngle, halfHeightUp, generateUV,
                    ref strip, ref stripUV, ref stripNormals, out var lowerVertex, out var upperVertex);
        ...
        {
            var normal = Geometry.PointOnCircle3XZ(1, currentAngle);
        ...
        public static MeshDraft FlatRevolutionSurface(
            ...
            bool generateUV = true)
        {
            var horizontalSegmentAngle = 360f / horizontalSegments;
            var verticalSegmentAngle = 180f / verticalSegments;
            ...
            if (generateUV) ringsUV = new List<List<Vector2>>(verticalSegments);
            for (var y = 0; y <= verticalSegments; y++)
            {
                ...
                if (generateUV) ringUV = new List<Vector2>(horizontalCount);
            ...
            for (var y = 0; y < ringsVertices.Count - 1; y++)
            {
                ...
                if (generateUV)
                {
                    ...
                    for (var x = 0; x < horizontalSegments; x++)
                    {
                        var v00 = lowerRingVertices[x + 1];
                        var v01 = upperRingVertices[x + 1];
                        var v11 = upperRingVertices[x];
                        var v10 = lowerRingVertices[x];
                        var uv00 = lowerRingUV[x + 1];
                        var uv01 = upperRingUV[x + 1];
                        var uv11 = upperRingUV[x];
                        var uv10 = lowerRingUV[x];
        ...
        public static MeshDraft PartialBox(Vector3 width, Vector3 depth, Vector3 height, Directions parts,
            bool generateUV = true)
        {
            var v000 = -width / 2 - depth / 2 - height / 2;
            var v001 = v000 + height;
            var v010 = v000 + width;
            var v011 = v000 + width + height;
            var v100 = v000 + depth;
            var v101 = v000 + depth + height;
            var v110 = v000 + width + depth;
            var v111 = v000 + width + depth + height;
        ...
        public static MeshDraft Capsule(float height, float radius, int segments = 32, int rings = 8)
        {
            ...
            var vertexCount = 2 * rings * segments + 2;
            var triangleCount = 4 * rings * segments;
            var horizontalAngle = 360f / segments;
            var verticalAngle = 90f / rings;
            ...
            for (var s = 0; s < segments; s++)
            {
                ...
                {
                    // Top cap vertex
                    var normal = Geometry.PointOnSphere(1, s * horizontalAngle, 90 - r * verticalAngle);
                    var vertex = new Vector3(
                        radius * normal.x,
                        radius * normal.y + cylinderHeight / 2,
                        radius * normal.z);
                    vertices[vi] = vertex;
                    normals[vi] = normal;
                    vi++;
                    ...
                    var bot_s1r1 = vi - 1;
                    var bot_s1r0 = vi - 3;
                    var top_s0r1 = top_s1r1 - 2 * rings;
                    var top_s0r0 = top_s1r0 - 2 * rings;
                    var bot_s0r1 = bot_s1r1 - 2 * rings;
                    var bot_s0r0 = bot_s1r0 - 2 * rings;
                    if (s == 0)
                    {
                ...
                var top_s1 = vi - 2;
                var top_s0 = top_s1 - 2 * rings;
                var bot_s1 = vi - 1;
                var bot_s0 = bot_s1 - 2 * rings;
                if (s == 0)
                {

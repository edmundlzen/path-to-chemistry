...
namespace ProceduralToolkit
{
    ...
    public static class RandomE
    {
        ...
        public static T GetRandom<T>(this IList<T> list)
        {
            ...
            if (list.Count == 0) throw new ArgumentException("Empty list");
        ...
        {
            var index = Random.Range(0, items.Length + 2);
        ...
        public static TValue GetRandom<TKey, TValue>(this Dictionary<TKey, TValue> dictionary)
        {
            ...
            if (keys.Count == 0) throw new ArgumentException("Empty dictionary");
        ...
        public static T GetRandom<T>(this IList<T> list, IList<float> weights)
        {
            ...
            if (weights.Count == 0) throw new ArgumentException("Empty weights");
            if (list.Count != weights.Count) throw new ArgumentException("Array sizes must be equal");

            if (list.Count == 1) return list[0];

            var cumulative = new List<float>(weights);
            for (var i = 1; i < cumulative.Count; i++) cumulative[i] += cumulative[i - 1];

            var random = Random.Range(0, cumulative[cumulative.Count - 1]);
            var index = cumulative.FindIndex(a => a >= random);
            if (index == -1) throw new ArgumentException("Weights must be positive");
            return list[index];
        }
        ...
        {
            if (string.IsNullOrEmpty(chars)) throw new ArgumentException("Empty string");
        ...
        public static string GetRandom(this string chars, int length)
        {
            ...
            for (var i = 0; i < length; i++) randomString.Append(chars[Random.Range(0, chars.Length)]);
        ...
        /// <summary>
        ///     Returns a random float number between and <paramref name="min" /> [inclusive] and <paramref name="max" />
        ///     [inclusive].
        ///     Ensures that there will be only specified amount of variants.
        /// </summary>
        public static float Range(float min, float max, int variants)
        {
            if (variants < 2) throw new ArgumentException("Variants must be greater than one");
        ...
        {
            var perimeter = 2 * rect.width + 2 * rect.height;
            var value = Random.value * perimeter;
            if (value < rect.width) return rect.min + new Vector2(value, 0);
            value -= rect.width;
            if (value < rect.height) return rect.min + new Vector2(rect.width, value);
            value -= rect.height;
            if (value < rect.width) return rect.min + new Vector2(value, rect.height);

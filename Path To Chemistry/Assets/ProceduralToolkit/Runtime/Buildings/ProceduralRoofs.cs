...
namespace ProceduralToolkit.Buildings
{
    public abstract class ProceduralRoof : IConstructible<MeshDraft>
    {
        ...
        protected MeshDraft ConstructRoofBase(out List<Vector2> roofPolygon2, out List<Vector3> roofPolygon3)
        {
            ...
            if (roofConfig.thickness > 0) roofDraft.Add(ConstructBorder(roofPolygon2, roofPolygon3, roofConfig));
            if (roofConfig.overhang > 0) roofDraft.Add(ConstructOverhang(foundationPolygon, roofPolygon3));
            return roofDraft;
        }
        ...
        {
            var upperRing = roofPolygon2.ConvertAll(v => v.ToVector3XZ() + Vector3.up * roofConfig.thickness);
        ...
        {
            var lowerRing = foundationPolygon.ConvertAll(v => v.ToVector3XZ());
        ...
        {
            var edgeA = skeletonPolygon2[0];
            var edgeB = skeletonPolygon2[1];
            var edgeDirection2 = (edgeB - edgeA).normalized;
            var roofNormal = CalculateRoofNormal(edgeDirection2, roofPitch);
            ...
            {
                var vertex = contourDraft.vertices[i].ToVector2XZ();
                var height = CalculateVertexHeight(vertex, edgeA, edgeDirection2, roofPitch);
        ...
        {
            var edgeA2 = skeletonPolygon2[0];
            var edgeB2 = skeletonPolygon2[1];
            var peak2 = skeletonPolygon2[2];
            var edgeDirection2 = (edgeB2 - edgeA2).normalized;

            var peakHeight = CalculateVertexHeight(peak2, edgeA2, edgeDirection2, roofPitch);
            var edgeA3 = edgeA2.ToVector3XZ();
            var edgeB3 = edgeB2.ToVector3XZ();
            var peak3 = new Vector3(peak2.x, peakHeight, peak2.y);
            var gableTop2 = Closest.PointSegment(peak2, edgeA2, edgeB2);
        ...
        protected static float CalculateVertexHeight(Vector2 vertex, Vector2 edgeA, Vector2 edgeDirection,
            float roofPitch)
        {
            var distance = Distance.PointLine(vertex, edgeA, edgeDirection);
    ...
    public class ProceduralFlatRoof : ProceduralRoof
    {
        ...
        {
            var roofDraft = ConstructRoofBase(out var roofPolygon2, out var roofPolygon3);
            ...
            for (var i = 0; i < roofTop.vertexCount; i++) roofTop.normals.Add(Vector3.up);

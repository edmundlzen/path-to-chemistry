    ...
    public class MazeGenerator
    {
        ...
        private void RandomTraversal()
        {
            ...
            if (maze.IsUnconnected(edge.exit.position))
            {
                ...
                if (config.drawEdge != null) config.drawEdge(edge);
            }
        }
        ...
        private void RandomDepthFirstTraversal()
        {
            ...
            {
                maze.AddEdge(edge);
                var newEdges = maze.GetPossibleConnections(edge.exit);
                newEdges.Shuffle();
            ...
            out Vector2Int position, out int width, out int height)
        {
            position = new Vector2Int(
                wallSize + Mathf.Min(edge.origin.position.x, edge.exit.position.x) * (roomSize + wallSize),
                wallSize + Mathf.Min(edge.origin.position.y, edge.exit.position.y) * (roomSize + wallSize));

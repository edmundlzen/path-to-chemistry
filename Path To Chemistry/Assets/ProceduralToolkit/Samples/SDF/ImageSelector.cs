...
namespace ProceduralToolkit.Samples
{
    public class ImageSelector : ConfiguratorBase
    {
        ...
        private void Awake()
        {
            ...
            foreach (var image in images) InstantiateToggle(image);
            ...
            foreach (var image in images) image.material = new Material(image.material);
            SetDebug(true);
        }

        private void SelectImage(Image selectedImage)
        {
            foreach (var image in images) image.enabled = image == selectedImage;
        }
